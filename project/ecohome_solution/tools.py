...
def get_weather_forecast(location: str, days: int = 3) -> Dict[str, Any]:
    ...
    for day in range(days):
        for hour in range(24):
            ...
            base_temp = 20 + 5 * random.random()
            if 6 <= hour <= 18:  # Daytime
                temp = base_temp + random.uniform(2, 8)
                conditions = ["sunny", "partly_cloudy", "cloudy"]
                solar_irradiance = random.uniform(300, 1000) if hour >= 8 and hour <= 16 else random.uniform(0, 200)
            else:  # Nighttime
                temp = base_temp - random.uniform(3, 7)
...
def get_electricity_prices(date: str = None) -> Dict[str, Any]:
    ...
    for hour in range(24):
        if 6 <= hour < 22:  # Peak hours (6 AM to 10 PM)
            if 16 <= hour < 21:  # Super peak (4 PM to 9 PM)
                rate = base_price * 2.5
                period = "super_peak"
                demand_charge = 0.05
            else:
                ...
                period = "peak"
                demand_charge = 0.03
        else:  # Off-peak hours (10 PM to 6 AM)
            rate = base_price * 0.7
            period = "off_peak"
            demand_charge = 0.0
